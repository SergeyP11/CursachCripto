...
class DEAL:
    ...
    block_size = 16
    _valid_key_sizes = (16, 24, 32)  # 128, 192, 256 бит

    def __init__(self, key: bytes):
        if len(key) not in self._valid_key_sizes:
            raise ValueError(
                "Ключ DEAL должен быть длиной 16, 24 или 32 байта "
                "(128/192/256 бит)"
            )
        self._setup_round_keys(key)

    def _setup_round_keys(self, key: bytes) -> None:
        
        des_keys = [key[i : i + 8] for i in range(0, len(key), 8)]
        self._des_rounds = [DES(k) for k in des_keys]
        ...
        self._round_keys = [
            self._des_rounds[i % len(self._des_rounds)] for i in range(rounds)
        ]
    ...
    def _feistel(self, block: bytes, encrypt: bool) -> bytes:
        if len(block) != self.block_size:
            raise ValueError("Блок DEAL должен быть длиной 16 байт (128 бит)")

        L = block[:8]
        R = block[8:]

        round_keys = self._round_keys if encrypt else reversed(self._round_keys)

        for des_cipher in round_keys:
            F = des_cipher.encrypt_block(R)
